...
function linamira_sanitize_class_list(string $class_names): string
{
...
function linamira_default_navigation_items(string $location): array
{
    $items = [
        'primary' => [
            ['label' => 'Acasă', 'url' => '/'],
            ['label' => 'Parfum pentru casă', 'url' => '/categorie-produs/parfum-pentru-casa/'],
            ['label' => 'Lumânări', 'url' => '/categorie-produs/lumanari-ceara-parfumata/'],
            ['label' => 'Difuzoare', 'url' => '/categorie-produs/difuzoare-de-arome/'],
            ['label' => 'Baie & duș', 'url' => '/categorie-produs/baie-dus/'],
            ['label' => 'Săpunuri', 'url' => '/categorie-produs/sapunuri/'],
            ['label' => 'Accesorii', 'url' => '/categorie-produs/accesorii-cadouri-mici/'],
        ],
        'footer_shop' => [
            ['label' => 'Parfum pentru casă', 'url' => '/categorie-produs/parfum-pentru-casa/'],
            ['label' => 'Lumânări & ceară parfumată', 'url' => '/categorie-produs/lumanari-ceara-parfumata/'],
            ['label' => 'Difuzoare de arome', 'url' => '/categorie-produs/difuzoare-de-arome/'],
            ['label' => 'Baie & duș', 'url' => '/categorie-produs/baie-dus/'],
            ['label' => 'Săpunuri & flori de săpun', 'url' => '/categorie-produs/sapunuri/'],
            ['label' => 'Accesorii & cadouri mici', 'url' => '/categorie-produs/accesorii-cadouri-mici/'],
        ],
        'footer_support' => [
            ['label' => 'Contact', 'url' => '/contact/'],
            ['label' => 'Livrare și plată', 'url' => '/livrare-si-plata/'],
            ['label' => 'Retur și rambursare', 'url' => '/retur-si-rambursare/'],
            ['label' => 'ANPC / SAL', 'url' => '/anpc-sal/'],
        ],
        'footer_legal' => [
            ['label' => 'Termeni și condiții', 'url' => '/termeni-si-conditii/'],
            ['label' => 'Politica de confidențialitate', 'url' => '/politica-de-confidentialitate/'],
            ['label' => 'Politica de cookies', 'url' => '/politica-cookies/'],
        ],
    ];
...
function linamira_navigation_locations(): array
{
    return [
        'primary' => __('Navigare principală', 'linamira'),
        'footer_shop' => __('Footer - Magazin', 'linamira'),
        'footer_support' => __('Footer - Asistență', 'linamira'),
        'footer_legal' => __('Footer - Legal', 'linamira'),
    ];
}

function linamira_normalize_menu_url(string $url): string
{
...
function linamira_render_default_navigation_menu(string $location, string $class_name, string $aria_label): string
{
    $items = linamira_default_navigation_items($location);

    if ([] === $items) {
        return '';
    }

    $nav_class = linamira_sanitize_class_list($class_name . ' wp-block-navigation');
    $output = sprintf(
        '<nav class="%s" aria-label="%s"><ul class="wp-block-navigation__container">',
        esc_attr($nav_class),
        esc_attr($aria_label)
    );
    ...
    foreach ($items as $item) {
        $output .= sprintf(
            '<li class="wp-block-navigation-item wp-block-navigation-link"><a class="wp-block-navigation-item__content" href="%s">%s</a></li>',
            esc_url(linamira_normalize_menu_url((string) $item['url'])),
...
function linamira_render_navigation_menu(string $location, string $class_name, string $aria_label): string
{
    if (has_nav_menu($location)) {
        $menu = wp_nav_menu(
            [
                ...
                'container_class' => linamira_sanitize_class_list($class_name . ' wp-block-navigation'),
                'depth' => 2,
                'echo' => false,
                'fallback_cb' => '__return_empty_string',
                'items_wrap' => '<ul id="%1$s" class="%2$s">%3$s</ul>',
                'linamira_navigation_menu' => true,
    ...
    return linamira_render_default_navigation_menu($location, $class_name, $aria_label);
}

add_filter('nav_menu_css_class', function (array $classes, WP_Post $item, stdClass $args): array {
    unset($item);

    if (! empty($args->linamira_navigation_menu)) {
...
function linamira_render_navigation_menu_block(array $attributes): string
{
    $location = sanitize_key((string) ($attributes['location'] ?? 'primary'));
    $class_name = linamira_sanitize_class_list((string) ($attributes['className'] ?? ''));
    $aria_label = trim((string) ($attributes['ariaLabel'] ?? ''));

    if ('' === $aria_label) {
        $locations = linamira_navigation_locations();
        $aria_label = (string) ($locations[$location] ?? __('Navigare', 'linamira'));
    }

    return linamira_render_navigation_menu($location, $class_name, $aria_label);
}

function linamira_render_logo_block(array $attributes): string
{
    $link_class = linamira_sanitize_class_list((string) ($attributes['className'] ?? 'lm-wordmark'));
    $image_class = linamira_sanitize_class_list((string) ($attributes['imageClassName'] ?? 'lm-logo-image'));
    $aria_label = trim((string) ($attributes['ariaLabel'] ?? __('Linamira acasă', 'linamira')));

    return sprintf(
        '<a class="%s" href="%s" aria-label="%s"><img class="%s" src="%s" alt="" width="1285" height="130" decoding="async" /></a>',
        esc_attr($link_class),
        esc_url(home_url('/')),
        esc_attr($aria_label),
        esc_attr($image_class),
        esc_url(get_theme_file_uri('assets/img/logo_linamira_bold.svg'))
    );
}

function linamira_render_anpc_sal_block(): string
{
    return sprintf(
        '<a href="https://reclamatiisal.anpc.ro/" target="_blank" rel="nofollow noopener noreferrer"><img src="%s" alt="%s" width="250" height="62" loading="lazy" decoding="async" /></a>',
        esc_url(get_theme_file_uri('assets/img/anpc/anpc-sal.png')),
        esc_attr__('Soluționarea Alternativă a Litigiilor', 'linamira')
    );
}
...
add_action('init', function (): void {
    register_block_type(
        ...
        [
            'api_version' => 2,
            'title' => __('LINAMIRA Navigation Menu', 'linamira'),
            'category' => 'theme',
            'attributes' => [
            ...
            'render_callback' => 'linamira_render_navigation_menu_block',
        ]
    );
    ...
    register_block_type(
        ...
        [
            'api_version' => 2,
            'title' => __('LINAMIRA Logo', 'linamira'),
            'category' => 'theme',
            'attributes' => [
                'ariaLabel' => ['type' => 'string'],
                'className' => ['type' => 'string'],
            ...
            'render_callback' => 'linamira_render_logo_block',
        ]
    );

    register_block_type(
        'linamira/anpc-sal-link',
        [
            'api_version' => 2,
            'title' => __('LINAMIRA ANPC SAL Link', 'linamira'),
            'category' => 'theme',
            'render_callback' => 'linamira_render_anpc_sal_block',
        ]
    );
});

function linamira_seed_default_navigation_menus(): void
{
    if (get_option('linamira_seeded_nav_menus')) {
        return;
    }
    ...
    foreach (linamira_navigation_locations() as $location => $label) {
        if (! empty($locations[$location])) {
            continue;
        }

        $menu_name = sprintf('LINAMIRA - %s', $label);
        ...
        if ([] === wp_get_nav_menu_items($menu_id)) {
            foreach (linamira_default_navigation_items((string) $location) as $item) {
                wp_update_nav_menu_item(
                    $menu_id,
                    0,
                    [
                        'menu-item-title' => (string) $item['label'],
                        'menu-item-url' => linamira_normalize_menu_url((string) $item['url']),
    ...
    update_option('linamira_seeded_nav_menus', time(), false);
}

add_action('after_switch_theme', 'linamira_seed_default_navigation_menus');
add_action('admin_init', 'linamira_seed_default_navigation_menus');
...
add_action('after_setup_theme', function (): void {
    register_nav_menus(linamira_navigation_locations());
